...
import { UserInterface, objectIsUserInterface } from "./types.js";
...
class PGUsers {
  ...
  getUsers = async (res: Express.Response, next: any) => {
    let client = null;
    try {
      client = await this.pool.connect();
      let outArray = [];
      const {rows} = await client.query("SELECT * FROM users");
      rows.forEach((element: unknown) => {
  ...
  insertUserReusable = async (userInfo: UserInterface, next: any) => {
    ...
    try {
      ...
      if (userInfo.id === null || userInfo.id === undefined) {
        ...
        const tempPwd: string = hash(password);
        delete userInfo.id;
        result = await this.pool.query("INSERT INTO users (login, pwd, nickname, email, roles, locale, active, reset_password) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *", 
          [userInfo.login, tempPwd, userInfo.nickname, userInfo.email, userInfo.roles, userInfo.locale, userInfo.active, userInfo.reset_password]);
      } else {
  ...
  insertUser = async (userInfo: UserInterface, res: Express.Response, next: any) => {
    let result = null;
    try {
  ...
  updateUser = async (userInfo: UserInterface) => {
    ...
    if (userInfo.id === null) {
      ...
      try {
        await this.pool.connect();
        return await this.pool.query("UPDATE users SET login = ($1), nickname = ($2), email = ($3), roles = ($4), locale = ($5), active = ($6), reset_password = ($7), refresh_token = ($8) WHERE id = ($9) RETURNING *", 
          [userInfo.login, userInfo.nickname, userInfo.email, userInfo.roles, userInfo.locale, userInfo.active, userInfo.reset_password, refreshToken, userInfo.id]);
      } catch(err) {
        console.error(err);
        throw err;
      }
  ...
  logoutUser = async (userId: number, res: Express.Response, next: any) => {
    let user = null;
    let client = null;
    try {
      client = await this.pool.connect();
      if (userId === null) {
        ...
        if (objectIsUserInterface(user)) {
          try {
            client.query("UPDATE users SET login = ($1), nickname = ($2), email = ($3), roles = ($4), locale = ($5), active = ($6), reset_password = ($7), refresh_token = ($8) WHERE id = ($9)",
              [user.login, user.nickname, user.email, user.roles, user.locale, user.active,user.reset_password, null, user.id]);
          } catch(err) {
            console.error(err);
            throw err;
          }
        }
        else throw new Error("Object is not a user")
      }
    } catch (err) {
      console.error(err);
      return next(err);
  ...
  deleteUser = async (id: number, res: Express.Response, next: any) => {
    try {
      await this.pool.connect();
  ...
  init = async (res: Express.Response, next: any) => {
    try {
      // let db = this.dbUtils.getDb();
      // try {
